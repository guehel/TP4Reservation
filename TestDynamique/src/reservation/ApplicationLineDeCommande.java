...
public class ApplicationLineDeCommande {
	...
	public static void main(String[] args) {
		...
		Services serviceWeb;
		serviceWeb = new FacadeServiceWeb();
				...
				if (type != 9) {
					...
					if (type == 1) {
						...
						if (reserver == 'O') {
							...
							do {
								int numeroChambreSelectionne = 0;
								do {
									System.out
											.println("Selectionner le numero de chambre:");
									numeroChambreSelectionne = sc.nextInt();
								} while (numeroChambreSelectionne <= 0
										|| numeroChambreSelectionne > ServicesWeb.NOMBRE_DE_CHAMBRES);
								chambreSelectionne = lChambreDTO[numeroChambreSelectionne - 1];
...
//								formulaire = chambreSelectionne.getFormulaire();
								...
								ReservationDTO[] reserva = serviceWeb.obtenirReservations(900 + numeroChambreSelectionne);
								for(ReservationDTO res :reserva){
